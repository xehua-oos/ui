...
-- 自适应DPI：手机版放大到120%，电脑版100%
Library.DPIScale = Library.IsMobile and 1.2 or 1.0
...
local function New(ClassName: string, Properties: { [string]: any }): any
    ...
    -- 胶囊按钮样式修改：自动为TextButton添加完全圆角和内边距
    if ClassName == "TextButton" and not Properties.NoCapsule then
...
-- ========== 下雪背景 ==========
local SnowBackground = New("Frame", {
    BackgroundTransparency = 1,
...
for i = 1, 50 do
    local flake = New("ImageLabel", {
        Image = "rbxassetid://6031069234",
        BackgroundTransparency = 1,
        ImageTransparency = 0.3 + math.random() * 0.5,
        Size = UDim2.fromOffset(8 + math.random(8), 8 + math.random(8)),
        Position = UDim2.fromScale(math.random(), math.random()),
        ZIndex = -1,
        Parent = SnowBackground,
    })
...
task.spawn(function()
    while true do
        ...
        for _, data in ipairs(Snowflakes) do
            local pos = data.Label.Position
            local newY = pos.Y.Scale + (data.Speed * dt * 0.05)
            if newY > 1.2 then
                newY = -0.2
                data.Label.Position = UDim2.fromScale(math.random(), newY)
            else
                data.Label.Position = UDim2.fromScale(pos.X.Scale, newY)
            end
        end
    end
end)
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:SetMax(Value)
            assert(Value > Slider.Min, "最大值不能小于当前的最小值。")
        ...
        function Slider:SetMin(Value)
            assert(Value < Slider.Max, "最小值不能大于当前的最大值。")
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile then
        local ToggleButton = Library:AddDraggableButton("切换界面", function()
            Library:Toggle()
        end)
