...
local ae = aa:CreateWindow{
    ...
    Background = "https://raw.githubusercontent.com/xehua-oos/WindUI/Image_1771151649001.jpg",  -- ← 添加这一行
    NewElements = true,
    HideSearchBar = false,
    OpenButton = {
        Title = "坤坤大帝 Hubris",
...
do
    ...
    ao:Paragraph{
        Title = "衍哥",
        Desc = "QQ: 913348285",
        Thumbnail = "https://q.qlogo.cn/headimg_dl?dst_uin=913348285&spec=640",
        ThumbnailSize = 80,
    }
    ...
    -- ===== 彩虹跑马灯（可调试） =====
    local marqueeSection = ao:Section{
        Title = "🌈 彩虹跑马灯 (可调试)",
        Box = true,
        BoxBorder = true,
    }

    local marqueeFrame = marqueeSection:Frame{
        Size = UDim2.new(1, 0, 0, 40),
        BackgroundTransparency = 1,
    }
    ...
    gradient.Parent = marqueeFrame

local aa
do
    local ae,af=pcall(function() return a.load'Z' end)
    if ae then aa=af else aa=loadstring(...)() end
end
    ...
    marqueeSection:Slider{
        Title = "滚动速度",
        Value = { Min = 0, Max = 0.1, Default = speed },
        Step = 0.001,
        IsTooltip = true,
    ...
    marqueeSection:Slider{
        Title = "透明度",
        Value = { Min = 0, Max = 1, Default = transparency },
        Step = 0.01,
        IsTooltip = true,
        Callback = function(v)
            transparency = v
            gradient.Transparency = NumberSequence.new(v)
        end
    }

    marqueeSection:Toggle{
        Title = "启用跑马灯",
        Value = enabled,
    ...
    heartbeat = game:GetService("RunService").Heartbeat:Connect(function()
        if enabled and marqueeFrame and marqueeFrame.Parent then
            local offset = gradient.Offset.X + speed
            if offset > 1 then offset = offset - 1 end
            gradient.Offset = Vector2.new(offset, 0)
        end
    end)

    marqueeFrame.AncestryChanged:Connect(function()
        if not marqueeFrame:IsDescendantOf(game) then
